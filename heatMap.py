...
def court(ax=None, color='black', lw=4, outer_lines=False, direction='up', short_three=False):
    ...
    if not short_three:
        ...
        corner_three_b = Rectangle(
            (22, -5.25), 0, np.sqrt(23.75**2-22.0**2)+5.25, linewidth=lw, color=color)
        ...
        corner_three_a = Rectangle((-22, -5.25), 0, 5.25, linewidth=lw,
                                   color=color)
        corner_three_b = Rectangle(
            (22, -5.25), 0, 5.25, linewidth=lw, color=color)
        # 3pt arc - center of arc will be the hoop, arc is 23'9" away from hoop
    ...
    if outer_lines:
        ...
        court_elements = court_elements + \
            [outer_lines, center_outer_arc, center_inner_arc]
    else:
...
def text_in_zone(string, zone, box_alpha=0.75, **kwargs):
    ...
    if zone == ('Less Than 8 ft.', 'Center(C)'):
        t = plt.text(0, 2, string, horizontalalignment='center',
                     verticalalignment='center', **kwargs)
        t.set_bbox(dict(color='white', alpha=box_alpha))
    elif zone[0] == '8-16 ft.':
        if zone[1] == 'Center(C)':
            t = plt.text(0, 12, string, horizontalalignment='center',
                         verticalalignment='center', **kwargs)
            t.set_bbox(dict(color='white', alpha=box_alpha))
        elif zone[1] == 'Right Side(R)':
            t = plt.text(12, 0, string, horizontalalignment='center',
                         verticalalignment='center', **kwargs)
            t.set_bbox(dict(color='white', alpha=box_alpha))
        ...
        if zone[1] == 'Center(C)':
            t = plt.text(0, 20, string, horizontalalignment='center',
                         verticalalignment='center', **kwargs)
            t.set_bbox(dict(color='white', alpha=box_alpha))
        elif zone[1] == 'Right Side(R)':
            t = plt.text(18, 7.3, string, horizontalalignment='center',
                         verticalalignment='center', **kwargs)
            t.set_bbox(dict(color='white', alpha=box_alpha))
        elif zone[1] == 'Left Side(L)':
            t = plt.text(-18, 7.3, string, horizontalalignment='center',
            ...
            t = plt.text(12, 15.5, string, horizontalalignment='center',
                         verticalalignment='center', **kwargs)
            t.set_bbox(dict(color='white', alpha=box_alpha))
        ...
        if zone[1] == 'Center(C)':
            t = plt.text(0, 28, string, horizontalalignment='center',
                         verticalalignment='center', **kwargs)
            t.set_bbox(dict(color='white', alpha=box_alpha))
        elif zone[1] == 'Right Side(R)':
            t = plt.text(27, 0, string, horizontalalignment='center',
                         verticalalignment='center', **kwargs)
            t.set_bbox(dict(color='white', alpha=box_alpha))
        elif zone[1] == 'Left Side(L)':
            t = plt.text(-27, 0, string, horizontalalignment='center',
            ...
            t = plt.text(19, 24, string, horizontalalignment='center',
                         verticalalignment='center', **kwargs)
            t.set_bbox(dict(color='white', alpha=box_alpha))
        elif zone[1] == 'Left Side Center(LC)':
            t = plt.text(-19, 24, string, horizontalalignment='center',
    ...
    if zone == ('Back Court Shot', 'Back Court(BC)'):
        t = plt.text(0, 39, string, horizontalalignment='center',
                     verticalalignment='center', **kwargs)
        t.set_bbox(dict(color='white', alpha=box_alpha))
...
def grantland_shotchart(shotchart, leagueavergae, ax=None, short_three=False, fg_range=[-9, 9]):
    LA = leagueavergae.loc[:, 'SHOT_ZONE_AREA':'FGM'].groupby(
        ['SHOT_ZONE_RANGE', 'SHOT_ZONE_AREA']).sum()
    LA['FGP'] = 1.0*LA['FGM']/LA['FGA']
    player = shotchart.groupby(
        ['SHOT_ZONE_RANGE', 'SHOT_ZONE_AREA', 'SHOT_MADE_FLAG']).size().unstack(fill_value=0)
    ...
    fig = plt.figure()
    poly_hexbins = plt.hexbin(
        x, y, gridsize=33, extent=[-25, 25, -6.25, 50-6.25], ec="#222222")
...
#    plt.figure(figsize=figsize,facecolor='white') #(0,0.17,0.57)
    ...
    court(ax, outer_lines=False, color='Grey', lw=.9,
          direction='down', short_three=short_three)
    ax.axis('off')
    # nba.plot.zones()
    # s=0.8747731368853422
    s = 0.85
    bins = np.concatenate(
        [[-np.inf], np.linspace(fg_range[0], fg_range[1], 200), [np.inf]])
    colors = [("#f0e43e"), ("#a7de71"), (0, 0.35, 0.3)]
    cm = LinearSegmentedColormap.from_list('my_list', colors, N=len(bins)-1)
    xy = s*np.array([np.cos(np.linspace(np.pi/6, np.pi*330/180, 6)),
                    np.sin(np.linspace(np.pi/6, np.pi*330/180, 6))]).T
    b = np.zeros((6, 2))
    ...
    for i in range(5):
        ...
        patches.append(Polygon(b))
        colors.append(i*50)
    plt.text(27.5, -9, 'Icy', horizontalalignment='center',
             verticalalignment='center')
    plt.text(22.5-2*0.76*4, -9, 'Hot', horizontalalignment='center',
             verticalalignment='center')
    ...
    b[:, 0] = xy[:, 0] + xc
    b[:, 1] = xy[:, 1] + yc
    plt.text(-24, -9, 'Frequency', horizontalalignment='center',
             verticalalignment='center')
    ...
    ax.text(-1, -8.5, shotchart.loc[0, 'PLAYER_NAME'], fontsize=11,
            color="#8c490a", horizontalalignment='center', verticalalignment='center')
    plt.show()
...
def shot_heatmap(df, sigma=1, log=False, player_pic=True, ax=None, cmap='jet'):
    ...
    '''
    n, _, _ = np.histogram2d(0.1*df['LOC_X'].values, 0.1*df['LOC_Y'].values, bins=[
                             500, 500], range=[[-25, 25], [-5.25, 44.75]])
    KDE = ndimage.filters.gaussian_filter(n, 10.0*sigma)
    N = 1.0*KDE/np.sum(KDE)
    if ax is None:
        ax = plt.gca(xlim=[30, -30], ylim=[-7, 43],
                     xticks=[], yticks=[], aspect=1.0)
    court(ax, outer_lines=True, color='black', lw=1.0, direction='down')
    ax.axis('off')
    if log:
        ax.imshow(np.rot90(np.log10(N+1)), cmap=cmap,
                  extent=[25.0, -25.0, -5.25, 44.75])
    else:
...
def shot_scatter(df, player_pic=True, ax=None, noise=True, **kwargs):
    ...
    if noise:
        ...
        Y = df.LOC_Y.values + \
            np.random.normal(loc=0.0, scale=1.5, size=len(df.LOC_Y.values))
    else:
        X = df.LOC_X.values
        Y = df.LOC_Y.values
    ...
    if player_pic:
        ...
        pic = players_picture(player_id)
        ax.imshow(pic, extent=[15, 25, 30, 37.8261])
        ax.text(21, 31, name, fontsize=18,
                horizontalalignment='center', verticalalignment='center')

    # ax.text(0,-7,'By: Doingthedishes',color='black',horizontalalignment='center',fontsize=20,fontweight='bold')
response = shotchartdetail.ShotChartDetail(season_nullable="2022-23", team_id=1610612746, player_id=201566, context_measure_simple="FGA"
                                           )
shotChartDetail = response.shot_chart_detail.get_data_frame()
avg = response.league_averages.get_data_frame()
print()
print(avg)
grantland_shotchart(shotChartDetail, avg)
