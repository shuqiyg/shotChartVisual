...
#    plt.figure(figsize=figsize,facecolor='white') #(0,0.17,0.57)
    ...
    court(ax,outer_lines=False,color='Grey',lw=.9,direction='down',short_three=short_three);
    ax.axis('off')
    #nba.plot.zones()
    ...
    for i in range(5):
        ...
        patches.append(Polygon(b))
        colors.append(i*50)
    plt.text(27.5,-9,'Icy',horizontalalignment='center',verticalalignment='center')
    plt.text(22.5-2*0.76*4,-9,'Hot',horizontalalignment='center',verticalalignment='center')
    ...
    b[:,0] = xy[:,0] + xc
    b[:,1] = xy[:,1] + yc
    plt.text(-24,-9,'Frequency',horizontalalignment='center',verticalalignment='center')
    ...
    ax.text(0,-8,shotchart.loc[0,'PLAYER_NAME'],fontsize=11,horizontalalignment='center',verticalalignment='center')
    plt.show()
...
def shot_scatter(df,player_pic=True,ax=None,noise=True,**kwargs):
    ...
    if player_pic:
        ...
        pic = players_picture(player_id)
        ax.imshow(pic,extent=[15,25,30,37.8261])
        ax.text(21,31,name,fontsize=18,horizontalalignment='center',verticalalignment='center')
    # ax.text(0,-7,'By: Doingthedishes',color='black',horizontalalignment='center',fontsize=20,fontweight='bold')
from nba_api.stats.endpoints import shotchartdetail
response = shotchartdetail.ShotChartDetail(season_nullable="2022-23", team_id=1610612761, player_id=1627832, context_measure_simple="FGA"
)
shotChartDetail = response.shot_chart_detail.get_data_frame()
avg = response.league_averages.get_data_frame()
print()
print(shotChartDetail)
